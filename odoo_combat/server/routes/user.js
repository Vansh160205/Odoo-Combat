...
const Listing = require("../models/Furniture")
...
router.patch("/:userId/:listingId", async (req, res) => {
  try {
    ...
    const listing = await Furniture.findById(listingId).populate("creator")
...
router.get("/:userId/properties", async (req, res) => {
  try {
    const { userId } = req.params
    const properties = await Furniture.find({ creator: userId }).populate("creator")
